...
export default function ManageNarrators() {
  const { narrators, movies, addNarrator, removeNarrator } = useMovies()
  const { t } = useLanguage()
  ...
  const submit = () => {
    ...
    if (narrators.some((n) => n.id === id)) return
  ...
  return (
    <div className="p-6 sm:p-8">
      <h1 className="font-display text-3xl tracking-wide text-parchment mb-1">{t.narrators_title}</h1>
      <p className="text-fog text-sm mb-6 font-body">{t.narrators_subtitle}</p>
      ...
      <div className="bg-reel border border-reel-line rounded-xl divide-y divide-reel-line">
        ...
        {!narrators.length && <p className="px-5 py-8 text-center text-fog text-sm">{t.narrators_empty}</p>}
      </div>
    </div>
  )
}
